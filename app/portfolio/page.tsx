export default function Portfolio() {
  const sections = [
    ["Education", "B.Tech Agricultural Engineering · Engineering student combining agriculture, software, AI, IoT and robotics."],
    ["Experience", "Founder of GKFXL · Full Stack Developer · AI Engineer · Startup Builder · End-to-end product development."],
    ["Currently Learning", "Advanced Next.js · AI Engineering · System Design · Cloud Architecture · PostgreSQL · Docker · DevOps · Machine Learning · LLMs · RAG"],
    ["Goals", "Build globally recognized software products, grow GKFXL, create impactful AI products, modernize agriculture through technology, help students and startups, and contribute to open source."],
  ];
  const projects = [
  ...
  return (
    <main className="page">
      ...
      <section className="section" id="skills"><div className="eyebrow">03 · SKILLS</div><h2>Tools I build with.</h2><div className="grid">{skills.map(([name,value])=><article className="item" key={name}><h3>{name}</h3><p>{value}</p></article>)}</div></section>
      ...
      {sections.slice(2,3).map(([name,value])=><section className="section" id="learning" key={name}><div className="eyebrow">06 · {name.toUpperCase()}</div><h2>Always improving.</h2><article className="item"><p>{value}</p></article></section>)}

      <section className="section" id="goals"><div className="eyebrow">07 · GOALS</div><h2>Dream · Build · Scale</h2><article className="item"><p>{sections[3][1]}</p></article></section>
