...
export default function Home() {
  ...
  return <main className="page"><div className="ambient ambient-one"/><div className="ambient ambient-two"/>
    ...
    <div className={`floating-ai ${chatOpen ? "is-open" : ""}`}><button className="floating-ai-button" onClick={() => setChatOpen(v => !v)} aria-label={chatOpen ? "Close Gokul AI" : "Open Gokul AI"}>{chatOpen ? "×" : "✦"}</button>{chatOpen && <div className="floating-chat"><div className="floating-chat-head"><div><span className="avatar">G</span><span><b>Gokul AI</b><small>Personal portfolio assistant</small></span></div><button onClick={() => setChatOpen(false)} aria-label="Close chat">×</button></div><div className="floating-chat-message"><span className="mini-avatar">G</span><p>{reply}</p></div><div className="prompt-row">{prompts.map(p => <button key={p} onClick={() => ask(p)} disabled={loading}>{p}</button>)}</div><form className="chatbox" onSubmit={e => { e.preventDefault(); ask(); }}><input value={message} onChange={e => setMessage(e.target.value)} placeholder="Ask anything…" disabled={loading}/><button type="submit" disabled={loading}>{loading ? "…" : "↑"}</button></form><div className="chat-foot">POWERED BY GEMINI · GOKUL.AI</div></div>}</div>
  </main>;
}
